        ...
        <form action="{{ route('admin.planos.update', $plano->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="tipo" id="tipo" class="mt-1 block w-full p-2 border rounded-md" required>
                    <option value="individual" {{ old('tipo', $plano->tipo) == 'individual' ? 'selected' : '' }}>
                        Individual</option>
                    <option value="familiar" {{ old('tipo', $plano->tipo) == 'familiar' ? 'selected' : '' }}>Familiar
                    </option>
            ...
            <div class="mb-4">
                ...
                <textarea name="cobertura" id="cobertura" rows="6" class="mt-1 block w-full p-2 border rounded-md"
                    required>{{ old('cobertura', implode(', ', json_decode($plano->cobertura, true) ?? explode(',', $plano->cobertura))) }}</textarea>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="faixaetaria" id="faixaetaria"
                    value="{{ old('faixaetaria', $plano->faixaetaria) }}"
                    class="mt-1 block w-full p-2 border rounded-md" required>
                @error('faixaetaria')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input type="text" name="preco" id="preco"
                    value="{{ old('preco', number_format($plano->preco, 2, ',', '.')) }}"
                    class="mt-1 block w-full p-2 border rounded-md" required>
                @error('preco')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            ...
            <div class="mt-6">
                <button type="submit"
                    class="px-4 py-2 bg-green-600 text-white rounded-md shadow hover:bg-green-700">Salvar
                    Alterações</button>
                <a href="{{ route('admin.planos.index') }}"
                    class="ml-4 px-4 py-2 bg-gray-600 text-white rounded-md shadow hover:bg-gray-700">Cancelar</a>
            </div>
        </form>
    </div>
</x-app-layout>
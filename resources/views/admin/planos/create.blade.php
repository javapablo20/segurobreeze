        ...
        <form action="{{ route('admin.planos.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="tipo" id="tipo" class="mt-1 block w-full p-2 border rounded-md" required>
                    ...
                    <option value="coletivo_empresarial" {{ old('tipo') == 'coletivo_empresarial' ? 'selected' : '' }}>
                        Coletivo Empresarial</option>
                    <option value="coletivo_adesao" {{ old('tipo') == 'coletivo_adesao' ? 'selected' : '' }}>Coletivo por
                        Adesão</option>
                </select>
                @error('tipo')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="cobertura" id="cobertura" value="{{ old('cobertura') }}"
                    class="mt-1 block w-full p-2 border rounded-md" required>
                @error('cobertura')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input type="text" name="faixaetaria" id="faixaetaria" value="{{ old('faixaetaria') }}"
                    class="mt-1 block w-full p-2 border rounded-md" required>
                @error('faixaetaria')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror

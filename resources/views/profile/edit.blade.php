    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    @forelse ($apolices as $apolice)
                        <div class="border-b border-gray-200 py-2">
                            ...
                            <p><strong>Data de Vencimento:</strong>
                                {{ \Carbon\Carbon::parse($apolice->datafim)->format('d/m/Y') }}</p>
                            ...
                            <div class="flex items-center space-x-4 mt-2">
                                @if ($apolice->status === 'ativa')
                                    <form action="{{ route('cancelar.apolice', $apolice->id) }}" method="POST"
                                        onsubmit="return confirm('Tem certeza que deseja cancelar esta apólice?')">
                                        @csrf
                                    ...
                                    <form action="{{ route('apolice.renovar', $apolice->id) }}" method="GET" class="inline">
                                        <button type="submit"
                                            class="bg-yellow-500 text-white px-6 py-2 rounded hover:bg-yellow-600 transition duration-300">
                                            Renovar Apólice
                                        </button>
                                    </form>

                                    <!-- Botão de Gerar Apólice -->
                                    <a href="{{ route('apolice.gerar', $apolice->id) }}"
            ...
            <div class="mt-4 text-center">
                <a href="{{ route('historico.apolices') }}"
